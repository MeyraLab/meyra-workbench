...
export function TriageSection() {
  const rules = ["先丢 Inbox", "当下不整理", "用过才进 Core", "一周扫进 Compost", "Core 必须可复制"];
  const triageFlow = [
    { type: "放松型", icon: Coffee, action: "只看不存", channels: ["Spotify", "ASMR", "随便刷"], destination: "不存" },
    { type: "灵感型", icon: Lightbulb, action: "先收着", channels: ["X", "Pinterest", "Telegram"], destination: "→ Inbox" },
    { type: "学习型", icon: GraduationCap, action: "要提炼", channels: ["微信读书", "Substack", "YouTube 大V"], destination: "→ NotebookLM" },
  ...
  return (
    <SectionCard
      title="C. Triage"
      instructions={["看完就分流", "按类型走", "放松的不记"]}
    >
      <div className="space-y-4">
        <p className="os-label">Next</p>
        {triageFlow.map((flow) => {
        ...
        <div className="border border-line px-4 py-3">
          <p className="os-label mb-2">Rules</p>
